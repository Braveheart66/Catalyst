...
export function Pricing() {
  return (
    <RevealSection id="pricing" className="relative z-10 px-5 py-28 lg:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {plans.map((plan) => (
            <RevealItem key={plan.name}>
              ...
              >
                {plan.featured ? (
                  <p className="mb-3 inline-flex rounded-full border border-violet-400/55 bg-violet-500/20 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-violet-100">
                    Most Popular
                  </p>
                ) : null}

                <h3 className="font-heading text-2xl font-semibold text-zinc-100">{plan.name}</h3>
                <p className="mt-3 font-heading text-4xl font-bold tracking-tight text-zinc-50">
                  {plan.price}
                ...
                <ul className="mt-5 space-y-2 text-sm text-zinc-300">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2">
                      <span className="mt-[3px] text-emerald-300">v</span>
                      <span>{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="mt-6">
                  <MagneticButton
                    ...
                    className={`h-11 w-full justify-center border px-4 text-sm font-semibold ${
                      plan.featured
                        ? "border-violet-400/60 bg-violet-600 text-zinc-100"
